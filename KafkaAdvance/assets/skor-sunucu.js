 ...
 * Testleri:  http://localhost:8899/01-broker-depolama/1.1-test.html  (önerilen)
...
function hedefDosya(s) {
  ...
  if (!path.isAbsolute(dizin) || !dizin.startsWith(ROOT)) dizin = path.join(ROOT, dizin);
...
sunucu.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`     Testleri buradan aç:  http://localhost:${PORT}/01-broker-depolama/1.1-test.html`);
